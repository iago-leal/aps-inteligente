...
| D-10 | Correção de cauda aplicada **só** a P/I e IMC/I, com o par negativo e positivo derivados da própria LMS | RN-03, confirmado contra a implementação de referência `gigs`, que exclui explicitamente `hcfa` e `lhfa` do procedimento (§4 de `investigation.md`); **e, desde 27/07, confirmado na fonte primária** — as colunas `SD4` das planilhas de peso e IMC já trazem o valor com a cauda aplicada, e não o da LMS (T008) | Aplicar a todos os índices — produziria escores errados de estatura e perímetro cefálico nas caudas; não aplicar a nenhum — devolveria escores irrealistas em desnutrição e obesidade graves | 🟢 |
| D-10.1 | A metade negativa de D-10 — a cauda **não** se aplica a C-E/I e PC/I — é exercitada em acervo **sintético** com `L ≠ 1`, nunca no dado real da OMS | Achado de T008: nesses dois indicadores `L = 1` em todas as tabelas, e com `L = 1` a LMS já é linear em `z` de passo `SD3 − SD2`, de modo que corrigir e não corrigir dão o mesmo número (diferença medida: 1e-14). O dado real é silencioso a respeito | Provar pelo dado real — passaria com a implementação certa e com a errada, que é o pior modo de falha de um teste. Vale a nota de consequência: errar aqui é clinicamente inócuo, ao passo que omitir a cauda no peso desloca o escore em até 10,4 unidades de IMC | 🟢 |
