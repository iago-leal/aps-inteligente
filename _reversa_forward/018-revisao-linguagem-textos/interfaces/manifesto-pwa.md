...
**Restrição dura.** O manifesto tem um teto prático de comprimento útil: descrição longa é truncada na apresentação de instalação, com corte variável por plataforma. A reescrita deve caber no que hoje existe e não aproveitar a revisão para alongar; encurtar é livre. 🟢

> **Correção de 27/07, na execução de T022.** Este parágrafo dizia "78 caracteres", e a medição encontrou **81**. Era número escrito em prosa, do tipo que L-13 tirou de circulação neste projeto — e a consequência prática seria pior que o engano: o verificador nasceria vermelho sobre um texto que não tem defeito nenhum, e a primeira reação de quem o visse seria afrouxá-lo. O teste `tests/unit/textos/manifesto.test.ts` passou a comparar contra o comprimento **medido** antes da revisão, com a constante nomeada e datada, em vez de contra a cifra citada daqui. A regra que o contrato enuncia continua a mesma, e é ela que vale: não alongar.
