...
| W013 | roadmap **D-11** (🟡); RN-09; ação T024 | O teto publicado é o do servidor que de fato atendeu. **A observar no primeiro deploy**, lendo `teto_de_conexoes` em produção: valor na casa da centena baixa confirma que a rota fala com a instância de cálculo, e a D-11 sobe a 🟢; valor na casa dos milhares indica agrupador de conexões no caminho, e então a ressalva da RN-08 deixa de ser precaução e vira o fato central a documentar | observação pós-deploy | Teto de ordem incompatível com a contagem do mesmo corpo; contagem excedendo o teto em produção, que seria a prova de que os dois números descrevem camadas diferentes e quebraria a asserção de não-excedência do W003 sem defeito de código |
