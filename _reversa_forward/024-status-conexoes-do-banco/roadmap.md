...
| D-11 | O teto publicado é o que o servidor alcançado reporta, seja ele a instância de cálculo ou o que estiver à frente dela | Não havia como decidir isto sem a `DATABASE_URL` de produção, que não está nesta máquina. A verificação era observável **depois** do deploy, pelo próprio campo, e foi: em 2026-08-11, sobre o commit `08636e3`, a produção devolveu `teto_de_conexoes: 112` e `conexoes_abertas: 1`. Centena baixa, e não milhar, de modo que a rota fala com a instância de cálculo da Neon e os dois números descrevem a mesma camada. A ressalva da RN-08 permanece escrita, agora como precaução e não como suspeita | fixar no contrato que o número é o da instância de cálculo, o que seria afirmação não verificada | 🟢 |
...
| 2026-08-11 | Primeiro deploy: a D-11 sobe a 🟢 com `teto_de_conexoes: 112` lido em produção. Nenhuma outra decisão muda | reversa |
