...
> Gerado pelo Reversa em 2026-07-19. Este arquivo é o contrato do registro de bugs deste projeto.
...
Racional da closure policy: fase 1 do produto sem usuários externos e **sem telemetria por
design** (ADR 0007) — uma janela de observação de recorrência em produção é inexequível sem
canal de observação. O gate de resolução é, portanto, o dos testes de regressão passando +
veredito de spec (`local-software`), coerente com a doutrina BUG_VIRA_TESTE (Princípio VII).
Gatilho de revisão desta policy: chegada de usuários externos regulares ou adoção de
telemetria concreta no `RelatorDeErros`.
